...
import "ghc" GHC.Utils.Outputable (showSDocUnsafe)
...
import "ghc-lib-parser" GHC.Utils.Outputable hiding (showSDocUnsafe)
#  if !defined (GHC_901)
    ...
    [file] -> do
      ...
      whenJust flags $ \flags ->
         case parse file (flags `gopt_set` Opt_KeepRawTokenStream) s of
            PFailed err -> putStrLn $ "Parse failed at " 
            POk _ m -> print $ showSDocUnsafe $ showAstData BlankSrcSpan BlankEpAnnotations m
